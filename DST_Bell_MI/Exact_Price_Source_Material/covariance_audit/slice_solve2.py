#!/usr/bin/env python3
"""
slice_solve2.py — TWO-SLICE COVARIANT REGISTER (patch of extract_pattern_rho.py,
the true producer of the paper's 16x16 solution, md=0.18983=1.375x floor).
TWO_SLICE=1 adds a second melody family pinned on the a=45deg slice:
trajectories evolved at (45deg, 45deg+Delta), same +-EPS band per angle.
TWO_SLICE=0 reproduces the original single-slice solve (anchor check).
Outputs pattern_rho.json / pattern_rho.png — never touches pattern.json.
Canon: TWO_SLICE=1 NSH=16 NW=2048 NANGF=41 EPS=0.01 CORES=56 python slice_solve2.py
...
TWO_SLICE = int(os.environ.get("TWO_SLICE", 0))
...
if __name__ == "__main__":
    ...
    for si, sc in enumerate(SCALES):
        ...
        if TWO_SLICE:
            ...
            for ai, td in enumerate(angF):
                for xa in xi:
                    for xb in xi:
                        jobs.append((w, xa, xb, a2, a2 + td*_deg))
                        meta.append(("g", si, ai))
    ...
    Eg = np.zeros((NANGF, NV)) if TWO_SLICE else None
    SGA2 = np.sign(np.cos(2*(lam_c - 45.0*_deg)))
    cp = {}; cf = {}; cg = {}
    for (kind, si, idx), o in zip(meta, outs):
        for ti in range(NT):
            ...
            if kind == "p":
                ...
                j = cg.setdefault((si, idx, ti), 0)
                tr = (45.0 + angF[idx])*_deg
                ob = np.sign(np.cos(2*(lam_c+np.pi/2-tr)))
                Eg[idx, col_base + j % NVo] = np.sum(o[ti]*SGA2*ob)*DL
                cg[(si, idx, ti)] += 1
    Svec = np.zeros(NV)
    ...
    if TWO_SLICE:
        for k in idxC:
            tgt = -np.cos(2*angF[k]*_deg)
            r = np.zeros(nv); r[:NV] = Eg[k]; rows.append(r); bub.append(tgt+EPS)
            r = np.zeros(nv); r[:NV] = -Eg[k]; rows.append(r); bub.append(-tgt+EPS)
    ...
    if TWO_SLICE:
        resid2 = (Eg@rho) - (-np.cos(2*angF*_deg))
        print(f"  a=45 slice worst in-register residual: {np.abs(resid2).max():.4f}")
    ...
    json.dump(dict(eps=EPS, md=float(md), angles=angF.tolist(),
                   ...
                   rho=rho.tolist(), nsh=NSH, nw=NW, two_slice=TWO_SLICE),
              open("pattern_rho.json","w"), indent=1)
    ...
    fig.tight_layout(); fig.savefig("pattern_rho.png", dpi=150)
    print("saved pattern_rho.png + pattern_rho.json")
